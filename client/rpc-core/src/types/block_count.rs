...
mod tests {
	...
	#[test]
	fn block_count_to_u64_saturates_decimal_input() {
		// The exact value that crashed `eth_feeHistory` in production: `2^64` as a
		// decimal string, which this crate's visitor parses decimal-first.
		let bc: BlockCount = serde_json::from_str(r#""18446744073709551616""#).unwrap();
		assert_eq!(u64::from(bc), u64::MAX);
	}
}
